...
class MockMT5SymbolInfo:
    ...
    def __init__(self, symbol: str):
        self.name = symbol
        self.digits = 2
        self.spread = 15  # 15 points = 1.5 pips
        self.ask = 2000.50
        self.bid = 2000.35
        self.volume_min = 0.01
        self.volume_max = 100.0
        self.volume_step = 0.01
        self.trade_contract_size = 100.0  # Gold contract size
        self.point = 0.01
...
class MockMT5:
    ...
    def positions_get(cls, symbol: Optional[str] = None, group: Optional[str] = None, ticket: Optional[int] = None) -> Tuple[Any, ...]:
        ...
        for t_id, pos in cls._positions.items():
            ...
            multiplier = 100.0  # Contract size
    ...
    def copy_rates_from_pos(cls, symbol: str, timeframe: int, start_pos: int, count: int) -> Optional[Any]:
        import numpy as np
        # Generate mock rates mimicking a slight bullish consolidation/trend
        # Fields: time, open, high, low, close, tick_volume, spread, real_volume
        rates = []
        base_time = 1717934400 - (count * 3600)
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if settings.MT5_LOGIN <= 0:
                app_logger.warning("MT5_LOGIN is 0 — no account credentials configured.")
                return False

        ...
        if settings.AI_VALIDATION_ENABLED and (anthropic_client or bedrock_client):
            try:
                ...
                prompt = (
                    ...
                    f"2. If BUY/SELL: Calculate entry (near current ask for buy, bid for sell), sl (approx 1.5x-2.5x ATR away from entry, or $2.00-$6.00 range), tp1 (1:1.5 risk/reward), tp2 (1:2.0+ risk/reward).\n"
                    f"3. If HOLD: Set action to HOLD, and still suggest a hypothetical entry (current mid price), sl, tp1, tp2 for reference.\n"
                    f"4. Provide a confidence level between 0.0 and 1.0 (BUY/SELL should only be suggested if confidence >= 0.70).\n"
                    f"5. Provide a brief professional technical reasoning.\n"
